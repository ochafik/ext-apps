...
interface AppState {
  ...
  sizeUpdateIntervalId: ReturnType<typeof setInterval> | null;
  logFilter: string;
}
...
const state: AppState = {
  ...
  sizeUpdateIntervalId: null,
  logFilter: "all",
};

// 1x1 red PNG for testing (used in multiple places)
const TEST_RED_PNG =
...
app.onteardown = async (params) => {
  ...
  if (state.sizeUpdateIntervalId) {
    clearInterval(state.sizeUpdateIntervalId);
    state.sizeUpdateIntervalId = null;
  }
...
// Event delegation for log entry toggling (avoids re-adding listeners on each render)
...
sendMessageImageBtn.addEventListener("click", async () => {
  try {
    const result = await app.sendMessage({
      role: "user",
      content: [{ type: "image", data: TEST_RED_PNG, mimeType: "image/png" }],
    });
    logEvent("send-message-result", result);
  } catch (e) {
    logEvent("error", e);
...
// Update current size periodically (store ID for cleanup in onteardown)
state.sizeUpdateIntervalId = setInterval(updateCurrentSize, 1000);
