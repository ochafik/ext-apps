...
interface UIResource {
   ...
   *
   * Declares the security and rendering policy for this resource at
   * discovery time (i.e. in the `resources/list` response).  Hosts MAY
   * use it before issuing `resources/read` — for example to pre-validate
   * CSP or to decide whether to offer the resource to the user.
   *
   * Servers MAY omit this entirely and attach `_meta.ui` on the content
   * item returned by `resources/read` instead (or in addition).  When
   * present in both places the content-item metadata takes precedence
   * on a per-field basis (see below).
   */
  _meta?: {
    ui?: UIResourceMeta;
  }
...
> **Where to put `_meta.ui`?**  `UIResourceMeta` is accepted in two places:
>
> | Location | When it is visible | Typical use |
> |---|---|---|
> | Resource descriptor (`resources/list`) | Discovery time, before content is fetched | Static policy declaration; pre-validation by hosts |
> | Content item (`resources/read`) | After the resource is read | Per-read policy; dynamic CSP/permissions tied to the content |
>
> Servers SHOULD place `_meta.ui` on the content item returned by `resources/read`, because that is the only location where the metadata travels together with the HTML it protects and can be varied per read.
> Servers MAY additionally (or instead) include it on the resource descriptor; hosts MAY use it for early validation.
>
> When metadata appears in **both** locations the host MUST merge them on a per-field basis, with the content-item value taking precedence for any field that is present in both.
...
// resources/read response for UI resource
{
  contents: [{
    ...
    _meta?: {
      ui?: {
        // Same UIResourceMeta shape as on the resource descriptor.
        // When present here it takes precedence over the descriptor-level value.
        csp?: {
...
```typescript
// `resource` is extracted from the `contents` of the `resources/read` result.
// Per-field merge: content-item _meta.ui overrides any same-keyed value in the
// resource descriptor (from `resources/list`).
